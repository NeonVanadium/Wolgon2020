...
class Label implements IPositioner {
	...
	public void wrapText(Graphics g) { 
		...
		int availableSpace = zone.getWidth() - (2 * AWolgonPanel.BUFFER); //available horizontal space in the zone, in pixels
		...
		while(indexOfEither(text, ' ', '\n', lastSpace + 1) != -1) {
			...
			if(text.charAt(lastSpace) == '\n' || m.stringWidth(lineAtNextSpace) + (AWolgonPanel.BUFFER) >= availableSpace) { 
	...
	private int getAlignedVerticalPosition() {
		...
		int halfLineHeight = (int) (AWolgonPanel.DEFAULT_FONT_SIZE / 2);
